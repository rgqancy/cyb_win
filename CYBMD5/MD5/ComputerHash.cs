namespace MD5
{
    ...
    internal class ComputerHash
    {
        private uint[] CRC32Table = new uint[] { 
        ...
        public int GetCRC32(byte[] bytes)
        {
            ...
            {
                maxValue = ((maxValue >> 8) & 0xffffff) ^ this.CRC32Table[(int) ((IntPtr) ((maxValue ^ bytes[i]) & 0xff))];
            }
            uint num4 = maxValue ^ uint.MaxValue;
            return (int) num4;
        }

        public string GetCRC32String(string sFileName)
        {
            ...
            try
            {
                ...
                for (long i = 0L; i < length; i += 1L)
                {
                    ...
                    maxValue = ((maxValue >> 8) & 0xffffff) ^ this.CRC32Table[(int) ((IntPtr) ((maxValue ^ num3) & 0xff))];
                }
                str = (maxValue ^ uint.MaxValue).ToString("X");
                stream.Close();
            }
            catch (Exception)
            {
        ...
        public string GetHashString(string sFileName, string sHashAlgorithm)
        {
            if (!File.Exists(sFileName))
            {
                return string.Empty;
            ...
            try
            {
                FileStream inputStream = new FileStream(sFileName, FileMode.Open, FileAccess.Read);
                HashAlgorithm algorithm = null;
                string str2 = sHashAlgorithm;
                if (str2 != null)
                {
                    if (!(str2 == "MD5"))
                    {
            ...
            Label_0093:
                if (algorithm != null)
                {
                    byte[] buf = algorithm.ComputeHash(inputStream);
                    str = this.ByteArrayToHexString(buf);
                }
                inputStream.Close();
            }
            catch (Exception)
        ...
        public string MD5File(string sFileName)
        {
            if (!File.Exists(sFileName))
            {
                return string.Empty;
            ...
            {
                FileStream inputStream = new FileStream(sFileName, FileMode.Open, FileAccess.Read);
                byte[] buf = MD5.Create().ComputeHash(inputStream);
                inputStream.Close();
                str = this.ByteArrayToHexString(buf);
            }
            catch (Exception)
        ...
        public string SHA1File(string sFileName)
        {
            if (!File.Exists(sFileName))
            {
                return string.Empty;
            ...
            {
                FileStream inputStream = new FileStream(sFileName, FileMode.Open, FileAccess.Read);
                byte[] buf = SHA1.Create().ComputeHash(inputStream);
                str = this.ByteArrayToHexString(buf);
                inputStream.Close();
            }
            catch (Exception)
